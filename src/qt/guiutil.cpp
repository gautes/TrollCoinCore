...
bool parseTrollcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!TrollcoinUnits::parse(TrollcoinUnits::TRC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatTrollcoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(TrollcoinUnits::format(TrollcoinUnits::TRC, info.amount, false, TrollcoinUnits::separatorNever));
        paramCount++;
    }
