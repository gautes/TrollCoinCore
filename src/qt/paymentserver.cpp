...
const int TROLLCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString TROLLCOIN_IPC_PREFIX("trollcoin:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(TROLLCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // trollcoin: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    Q_FOREACH (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(TROLLCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(TROLLCOIN_IPC_CONNECT_TIMEOUT);
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(TROLLCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // trollcoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
