...
Then, to tunnel a SSL connection on 29243 to a RPC server bound on localhost on port 18332 do:

    stunnel -d 29243 -r 127.0.0.1:18332 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ProxyPass http://127.0.0.1:9243/
        ProxyPassReverse http://127.0.0.1:9243/
...
- #7209 `f3ad812` test: don't override TROLLCOIND and TROLLCOINCLI if they're set (Wladimir J. van der Laan)
...
- #6401 `6db53b4` Add TROLLCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
...
- #6504 `b6fee6b` Rationalize currency unit to "TRC" (Ross Nicoll)
