...
The RPM spec file provided here is for Trollcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://trollcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Trollcoin git repository](https://github.com/trollcoin/trollcoin) but are not part of
...
Currently only `Source10` is used, for the example `trollcoin.conf` file.
...
in the trollcoin git repository. At present only an SVG version of the Trollcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the trollcoin
...
needed when 0.12.1 is released, a proper fix is already in the Trollcoin git
master branch.
...
idea to build Trollcoin Core against a static version of that library compiled
...
The `trollcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `trollcoin-server` package. At this point, most RPM based
...
The `trollcoin-server` package should have SELinux support. How to properly do
...
Trollcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/trollcoin/). He has been
packaging Trollcoin for Fedora at least since 2012.
...
4. I renamed the `trollcoin` package that contains the Qt GUI to `trollcoin-core` as
that appears to be how the general population refers to it, in contrast to
`trollcoin-xt` or `trollcoin-classic`. I wanted to make sure the general population
